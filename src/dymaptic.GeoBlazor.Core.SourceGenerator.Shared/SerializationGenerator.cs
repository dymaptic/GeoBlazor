        ...
        bool isPro, bool isTest)
    {
        try
        {
            ...
            StringBuilder classBuilder = new($$"""
                                               ...
                                               /// </summary>
                                               internal partial class {{className}}: ISerializationData
                                               {
        ...
        bool isPro)
    {
        if (isPro)
        {
            return """
                       ...
                       /// </summary>
                       public async Task<T?> ReadJsProtobufCollectionStreamReference<T>(IJSStreamReference jsStreamReference, 
                           Type returnType, long maxAllowedSize = 1_000_000_000)
                       {
                            return await _coreData.ReadJsProtobufCollectionStreamReference<T>(jsStreamReference, returnType, maxAllowedSize);
                       }
                ...
                readJsProtoCollectionStreamRefMethod.AppendLine(
                    $$"""
                                  case "{{protoSerializableType}}":
                                      ...
                                      return (T?)(object?){{variableName}}?.Items?.Select(i => i.FromSerializationRecord());
                      """);
