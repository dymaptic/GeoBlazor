...
export async function buildDotNetMapViewTakeScreenshotOptionsGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        let { buildJsMapViewTakeScreenshotOptions } = await import('./mapViewTakeScreenshotOptions');
        jsComponentRef = await buildJsMapViewTakeScreenshotOptions(jsObject, layerId, viewId);
    }
    ...
    if (hasValue(jsObject.area)) {
        ...
        dotNetMapViewTakeScreenshotOptions.area = await buildDotNetMapViewTakeScreenshotOptionsArea(jsObject.area, layerId, viewId);
    }
    if (hasValue(jsObject.format)) {
        let { buildDotNetFormat } = await import('./format');
        dotNetMapViewTakeScreenshotOptions.format = await buildDotNetFormat(jsObject.format, layerId, viewId);
    }
