...
export async function buildDotNetWebsceneInitialViewPropertiesGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.spatialReference)) {
        ...
        dotNetWebsceneInitialViewProperties.spatialReference = buildDotNetSpatialReference(jsObject.spatialReference, viewId);
    }
    
    if (hasValue(jsObject.timeExtent)) {
        let { buildDotNetTimeExtent } = await import('./timeExtent');
        dotNetWebsceneInitialViewProperties.timeExtent = buildDotNetTimeExtent(jsObject.timeExtent, viewId);
    }
    
    if (hasValue(jsObject.viewpoint)) {
        let { buildDotNetViewpoint } = await import('./viewpoint');
        dotNetWebsceneInitialViewProperties.viewpoint = buildDotNetViewpoint(jsObject.viewpoint, viewId);
    }
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetWebsceneInitialViewProperties.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetWebsceneInitialViewProperties.id)) {
        jsObjectRefs[dotNetWebsceneInitialViewProperties.id] ??= jsObject;
        arcGisObjectRefs[dotNetWebsceneInitialViewProperties.id] ??= jsObject;
    }
