            ...
            jsObjectRef, JSON.stringify(dnInstantiatedObject, function (key, value) {
                ...
                if (typeof value === 'object' && value !== null) {
                    if (seenObjects.has(value)) {
                        console.warn(`Circular reference in serializing type ITablesMixin detected at path: ${key}, value: ${value.__proto__?.declaredClass}`);
                        return undefined;
                    }
...
export async function buildDotNetITablesMixinGenerated(jsObject: any): Promise<any> {
    ...
    dotNetITablesMixin.id = lookupGeoBlazorId(jsObject);
