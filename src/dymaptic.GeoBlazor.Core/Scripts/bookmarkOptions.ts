...
export async function buildDotNetBookmarkOptions(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetBookmarkOptionsGenerated} = await import('./bookmarkOptions.gb');
    return await buildDotNetBookmarkOptionsGenerated(jsObject, layerId, viewId);
}
