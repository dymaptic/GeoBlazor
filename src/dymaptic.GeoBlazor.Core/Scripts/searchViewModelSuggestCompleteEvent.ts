...
export async function buildDotNetSearchViewModelSuggestCompleteEvent(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetSearchViewModelSuggestCompleteEventGenerated } = await import('./searchViewModelSuggestCompleteEvent.gb');
    return await buildDotNetSearchViewModelSuggestCompleteEventGenerated(jsObject, viewId);
}
