...
export default class MapViewWrapper extends BaseComponent {
    ...
    async buildDotNetHitTestResult(hitTestResult: HitTestResult, viewId: string): Promise<DotNetHitTestResult> {
        let results = await Promise.all(hitTestResult.results
            .map(r => this.buildDotNetViewHit(r as ViewHit, viewId)))
...
export async function buildJsMapView(dotNetObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    // n/a
}
export async function buildDotNetMapView(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    return 
}
