...
export async function buildDotNetKMLLayerView(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetKMLLayerViewGenerated} = await import('./kMLLayerView.gb');
    return await buildDotNetKMLLayerViewGenerated(jsObject, layerId, viewId);
}
