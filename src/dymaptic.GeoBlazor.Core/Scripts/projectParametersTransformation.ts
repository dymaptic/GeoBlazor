...
export async function buildDotNetProjectParametersTransformation(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetProjectParametersTransformationGenerated } = await import('./projectParametersTransformation.gb');
    return await buildDotNetProjectParametersTransformationGenerated(jsObject, layerId, viewId);
}
