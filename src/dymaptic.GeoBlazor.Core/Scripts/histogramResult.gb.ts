...
export async function buildDotNetHistogramResultGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.bins)) {
        ...
        dotNetHistogramResult.bins = await Promise.all(jsObject.bins.map(async i => await buildDotNetBin(i)));
    }
