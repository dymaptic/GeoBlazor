...
export async function buildDotNetOrderByInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetOrderByInfo.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetOrderByInfo.id)) {
        jsObjectRefs[dotNetOrderByInfo.id] ??= jsObject;
        arcGisObjectRefs[dotNetOrderByInfo.id] ??= jsObject;
    }
