...
export async function buildDotNetCoverageDescriptionV201BoundedBy(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetCoverageDescriptionV201BoundedByGenerated } = await import('./coverageDescriptionV201BoundedBy.gb');
    return await buildDotNetCoverageDescriptionV201BoundedByGenerated(jsObject, viewId);
}
