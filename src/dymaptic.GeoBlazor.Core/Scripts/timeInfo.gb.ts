...
export async function buildJsTimeInfoGenerated(dotNetObject: any, viewId: string | null): Promise<any> {
...
export async function buildDotNetTimeInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.fullTimeExtent)) {
        ...
        dotNetTimeInfo.fullTimeExtent = buildDotNetTimeExtent(jsObject.fullTimeExtent, viewId);
    }
    
    if (hasValue(jsObject.interval)) {
        let { buildDotNetTimeInterval } = await import('./timeInterval');
        dotNetTimeInfo.interval = await buildDotNetTimeInterval(jsObject.interval, viewId);
    }
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetTimeInfo.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetTimeInfo.id)) {
        jsObjectRefs[dotNetTimeInfo.id] ??= jsObject;
        arcGisObjectRefs[dotNetTimeInfo.id] ??= jsObject;
    }
