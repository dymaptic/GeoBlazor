...
export async function buildDotNetTextBoxInput(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetTextBoxInputGenerated } = await import('./textBoxInput.gb');
    return await buildDotNetTextBoxInputGenerated(jsObject, viewId);
}
