...
export async function buildDotNetIntervalNodeGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.op)) {
        dotNetIntervalNode.op = removeCircularReferences(jsObject.op);
    }
    
    if (hasValue(jsObject.qualifier)) {
        dotNetIntervalNode.qualifier = removeCircularReferences(jsObject.qualifier);
    }
    ...
    if (hasValue(jsObject.value)) {
        dotNetIntervalNode.value = removeCircularReferences(jsObject.value);
    }
