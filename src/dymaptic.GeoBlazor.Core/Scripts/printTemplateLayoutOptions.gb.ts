...
import PrintTemplateLayoutOptions = __esri.PrintTemplateLayoutOptions;
import {arcGisObjectRefs, hasValue, jsObjectRefs} from './arcGisJsInterop';
import {IPropertyWrapper} from './definitions';

export default class PrintTemplateLayoutOptionsGenerated implements IPropertyWrapper {
    public component: PrintTemplateLayoutOptions;
    public geoBlazorId: string | null = null;
    public viewId: string | null = null;
    public layerId: string | null = null;

    constructor(component: PrintTemplateLayoutOptions) {
    ...
    async getLegendLayers(): Promise<any> {
        if (!hasValue(this.component.legendLayers)) {
            return null;
        }
        
        let { buildDotNetILegendLayer } = await import('./iLegendLayer');
        return await Promise.all(this.component.legendLayers.map(async i => await buildDotNetILegendLayer(i)));
    }
    
    async setLegendLayers(value: any): Promise<void> {
        let { buildJsILegendLayer } = await import('./iLegendLayer');
        this.component.legendLayers = await Promise.all(value.map(async i => await buildJsILegendLayer(i, this.layerId, this.viewId))) as any;
    }
...
export async function buildJsPrintTemplateLayoutOptionsGenerated(dotNetObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    let { default: PrintTemplateLayoutOptionsWrapper } = await import('./printTemplateLayoutOptions');
    let printTemplateLayoutOptionsWrapper = new PrintTemplateLayoutOptionsWrapper(jsPrintTemplateLayoutOptions);
    printTemplateLayoutOptionsWrapper.geoBlazorId = dotNetObject.id;
    printTemplateLayoutOptionsWrapper.viewId = viewId;
    printTemplateLayoutOptionsWrapper.layerId = layerId;
    
    // @ts-ignore
    let jsObjectRef = DotNet.createJSObjectReference(printTemplateLayoutOptionsWrapper);
    jsObjectRefs[dotNetObject.id] = printTemplateLayoutOptionsWrapper;
