...
export async function buildDotNetMeasurementViewModel(jsObject: any, viewId: string | null): Promise<any> {
    let {buildDotNetMeasurementViewModelGenerated} = await import('./measurementViewModel.gb');
    return await buildDotNetMeasurementViewModelGenerated(jsObject, viewId);
}
