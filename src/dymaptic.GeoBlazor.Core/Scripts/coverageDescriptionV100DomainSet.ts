...
export async function buildDotNetCoverageDescriptionV100DomainSet(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetCoverageDescriptionV100DomainSetGenerated } = await import('./coverageDescriptionV100DomainSet.gb');
    return await buildDotNetCoverageDescriptionV100DomainSetGenerated(jsObject, viewId);
}
