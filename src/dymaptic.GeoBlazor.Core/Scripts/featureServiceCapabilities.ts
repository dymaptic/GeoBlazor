...
export async function buildDotNetFeatureServiceCapabilities(jsObject: any): Promise<any> {
    let { buildDotNetFeatureServiceCapabilitiesGenerated } = await import('./featureServiceCapabilities.gb');
    return await buildDotNetFeatureServiceCapabilitiesGenerated(jsObject);
}
