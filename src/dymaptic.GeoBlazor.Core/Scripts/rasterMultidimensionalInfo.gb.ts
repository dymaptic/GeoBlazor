...
export async function buildJsRasterMultidimensionalInfoGenerated(dotNetObject: any): Promise<any> {
    ...
    if (hasValue(dotNetObject.variables) && dotNetObject.variables.length > 0) {
        ...
        jsRasterMultidimensionalInfo.variables = await Promise.all(dotNetObject.variables.map(async i => await buildJsRasterMultidimensionalInfoVariables(i))) as any;
    }
