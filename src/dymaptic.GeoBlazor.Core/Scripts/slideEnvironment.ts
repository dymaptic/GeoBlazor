...
export async function buildDotNetSlideEnvironment(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetSlideEnvironmentGenerated } = await import('./slideEnvironment.gb');
    return await buildDotNetSlideEnvironmentGenerated(jsObject, layerId, viewId);
}
