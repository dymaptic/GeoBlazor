...
import { arcGisObjectRefs, jsObjectRefs, hasValue, removeCircularReferences, applyCaptureIdToElement } from './geoBlazorCore';
...
export async function buildDotNetSymbolTableElementInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.label)) {
        dotNetSymbolTableElementInfo.label = removeCircularReferences(jsObject.label);
    }
    
    if (hasValue(jsObject.preview)) {
        let referenceCaptureId = applyCaptureIdToElement(jsObject.preview);
        dotNetSymbolTableElementInfo.preview = {
    ...
    if (hasValue(jsObject.value)) {
        dotNetSymbolTableElementInfo.value = removeCircularReferences(jsObject.value);
    }
