...
export async function buildDotNetGeoJSONLayerGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.orderBy)) {
        let { buildDotNetOrderByInfo } = await import('./orderByInfo');
        dotNetGeoJSONLayer.orderBy = await Promise.all(jsObject.orderBy.map(async i => await buildDotNetOrderByInfo(i)));
    }
    ...
    if (hasValue(jsObject.trackInfo)) {
        let { buildDotNetTrackInfo } = await import('./trackInfo');
        dotNetGeoJSONLayer.trackInfo = await buildDotNetTrackInfo(jsObject.trackInfo);
    }
