...
export async function buildDotNetGraphApplyEditsGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.entityDeletes)) {
        ...
        dotNetGraphApplyEdits.entityDeletes = await Promise.all(jsObject.entityDeletes.map(async i => await buildDotNetGraphNamedObjectDeletes(i)));
    }
    if (hasValue(jsObject.relationshipDeletes)) {
        let { buildDotNetGraphNamedObjectDeletes } = await import('./graphNamedObjectDeletes');
        dotNetGraphApplyEdits.relationshipDeletes = await Promise.all(jsObject.relationshipDeletes.map(async i => await buildDotNetGraphNamedObjectDeletes(i)));
    }
