...
export async function buildDotNetGraphApplyEditsGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.entityAdds)) {
        dotNetGraphApplyEdits.entityAdds = removeCircularReferences(jsObject.entityAdds);
    }
    
    if (hasValue(jsObject.entityUpdates)) {
        dotNetGraphApplyEdits.entityUpdates = removeCircularReferences(jsObject.entityUpdates);
    }
    
    if (hasValue(jsObject.options)) {
        dotNetGraphApplyEdits.options = removeCircularReferences(jsObject.options);
    }
    
    if (hasValue(jsObject.relationshipAdds)) {
        dotNetGraphApplyEdits.relationshipAdds = removeCircularReferences(jsObject.relationshipAdds);
    }
    
    if (hasValue(jsObject.relationshipUpdates)) {
        dotNetGraphApplyEdits.relationshipUpdates = removeCircularReferences(jsObject.relationshipUpdates);
    }
