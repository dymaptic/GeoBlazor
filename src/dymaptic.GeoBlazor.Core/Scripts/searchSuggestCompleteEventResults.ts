...
export async function buildDotNetSearchSuggestCompleteEventResults(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetSearchSuggestCompleteEventResultsGenerated } = await import('./searchSuggestCompleteEventResults.gb');
    return await buildDotNetSearchSuggestCompleteEventResultsGenerated(jsObject, layerId, viewId);
}
