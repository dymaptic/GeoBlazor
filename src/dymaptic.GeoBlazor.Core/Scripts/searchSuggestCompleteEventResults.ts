...
export async function buildDotNetSearchSuggestCompleteEventResults(jsObject: any): Promise<any> {
    let { buildDotNetSearchSuggestCompleteEventResultsGenerated } = await import('./searchSuggestCompleteEventResults.gb');
    return await buildDotNetSearchSuggestCompleteEventResultsGenerated(jsObject);
}
