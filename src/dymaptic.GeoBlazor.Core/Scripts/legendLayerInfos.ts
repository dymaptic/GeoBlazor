...
export async function buildDotNetLegendLayerInfos(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetLegendLayerInfosGenerated} = await import('./legendLayerInfos.gb');
    return await buildDotNetLegendLayerInfosGenerated(jsObject, layerId, viewId);
}
