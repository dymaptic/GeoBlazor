...
export async function buildDotNetSunLight(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetSunLightGenerated } = await import('./sunLight.gb');
    return await buildDotNetSunLightGenerated(jsObject, layerId, viewId);
}
