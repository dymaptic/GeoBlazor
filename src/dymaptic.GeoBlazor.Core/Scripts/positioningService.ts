...
export async function buildDotNetPositioningService(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetPositioningServiceGenerated } = await import('./positioningService.gb');
    return await buildDotNetPositioningServiceGenerated(jsObject, layerId, viewId);
}
