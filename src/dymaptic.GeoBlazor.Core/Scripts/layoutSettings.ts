...
export async function buildDotNetLayoutSettings(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetLayoutSettingsGenerated } = await import('./layoutSettings.gb');
    return await buildDotNetLayoutSettingsGenerated(jsObject, viewId);
}
