...
export async function buildDotNetCloudyWeatherGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        if (hasValue(dotNetRef)) {
            try {
                dotNetCloudyWeather.id = await dotNetRef.invokeMethodAsync('GetId');
            } catch (e) {
                console.error('Error invoking GetId for CloudyWeather', e);
            }
        }
    }
    if (hasValue(dotNetCloudyWeather.id)) {
        jsObjectRefs[dotNetCloudyWeather.id] ??= jsObject;
        arcGisObjectRefs[dotNetCloudyWeather.id] ??= jsObject;
    }
