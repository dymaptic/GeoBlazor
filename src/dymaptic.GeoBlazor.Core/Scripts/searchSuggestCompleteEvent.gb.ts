...
export async function buildDotNetSearchSuggestCompleteEventGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        let { buildJsSearchSuggestCompleteEvent } = await import('./searchSuggestCompleteEvent');
        jsComponentRef = await buildJsSearchSuggestCompleteEvent(jsObject, layerId, viewId);
    }
    ...
    if (hasValue(jsObject.results)) {
        ...
        dotNetSearchSuggestCompleteEvent.results = await Promise.all(jsObject.results.map(async i => await buildDotNetSearchSuggestCompleteEventResults(i, layerId, viewId)));
    }
