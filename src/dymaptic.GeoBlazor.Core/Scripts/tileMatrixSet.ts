...
export async function buildDotNetTileMatrixSet(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetTileMatrixSetGenerated } = await import('./tileMatrixSet.gb');
    return await buildDotNetTileMatrixSetGenerated(jsObject, viewId);
}
