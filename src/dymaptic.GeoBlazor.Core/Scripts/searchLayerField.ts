...
export async function buildDotNetSearchLayerField(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetSearchLayerFieldGenerated } = await import('./searchLayerField.gb');
    return await buildDotNetSearchLayerFieldGenerated(jsObject, layerId, viewId);
}
