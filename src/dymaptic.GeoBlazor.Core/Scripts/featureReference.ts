...
export async function buildDotNetFeatureReference(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetFeatureReferenceGenerated } = await import('./featureReference.gb');
    return await buildDotNetFeatureReferenceGenerated(jsObject, viewId);
}
