...
export async function buildDotNetSnappingOptionsGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        dotNetSnappingOptions.id = geoBlazorId;
    } else if (hasValue(viewId)) {
        let dotNetRef = dotNetRefs[viewId!];
        dotNetSnappingOptions.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetSnappingOptions.id)) {
        jsObjectRefs[dotNetSnappingOptions.id] ??= jsObject;
        arcGisObjectRefs[dotNetSnappingOptions.id] ??= jsObject;
    }
