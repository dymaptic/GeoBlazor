...
export async function buildDotNetGamepadSettingsGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.devices)) {
        ...
        dotNetGamepadSettings.devices = await Promise.all(jsObject.devices.map(async i => await buildDotNetGamepadInputDevice(i)));
    }
