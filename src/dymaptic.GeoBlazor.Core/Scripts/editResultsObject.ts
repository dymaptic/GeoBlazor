...
export async function buildDotNetEditResultsObject(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetEditResultsObjectGenerated } = await import('./editResultsObject.gb');
    return await buildDotNetEditResultsObjectGenerated(jsObject, layerId, viewId);
}
