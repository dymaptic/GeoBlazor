...
export async function buildDotNetRainyWeather(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetRainyWeatherGenerated } = await import('./rainyWeather.gb');
    return await buildDotNetRainyWeatherGenerated(jsObject, viewId);
}
