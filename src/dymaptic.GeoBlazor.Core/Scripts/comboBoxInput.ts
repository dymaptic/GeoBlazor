...
export async function buildDotNetComboBoxInput(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetComboBoxInputGenerated } = await import('./comboBoxInput.gb');
    return await buildDotNetComboBoxInputGenerated(jsObject, viewId);
}
