...
export async function buildDotNetLevelLayerInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        dotNetLevelLayerInfo.id = geoBlazorId;
    } else if (hasValue(viewId)) {
        let dotNetRef = dotNetRefs[viewId!];
        dotNetLevelLayerInfo.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetLevelLayerInfo.id)) {
        jsObjectRefs[dotNetLevelLayerInfo.id] ??= jsObject;
        arcGisObjectRefs[dotNetLevelLayerInfo.id] ??= jsObject;
    }
