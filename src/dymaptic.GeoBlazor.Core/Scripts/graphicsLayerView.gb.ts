...
export default class GraphicsLayerViewGenerated extends BaseComponent implements IPropertyWrapper {
...
export async function buildDotNetGraphicsLayerViewGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.highlightOptions)) {
        let { buildDotNetHighlightOptions } = await import('./highlightOptions');
        dotNetGraphicsLayerView.highlightOptions = await buildDotNetHighlightOptions(jsObject.highlightOptions, viewId);
    }
    ...
    if (hasValue(geoBlazorId)) {
        ...
        if (hasValue(dotNetRef)) {
            try {
                dotNetGraphicsLayerView.id = await dotNetRef.invokeMethodAsync('GetId');
            } catch (e) {
                console.error('Error invoking GetId for GraphicsLayerView', e);
            }
        }
    }
