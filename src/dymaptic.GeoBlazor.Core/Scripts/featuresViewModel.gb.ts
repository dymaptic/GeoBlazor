...
export async function buildDotNetFeaturesViewModelGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    let dotNetFeaturesViewModel: any = {
        ...
        if (hasValue(jsObject.actions)) {
            let { buildDotNetActionBase } = await import('./actionBase');
            dotNetFeaturesViewModel.actions = await Promise.all(jsObject.actions.map(async i => await buildDotNetActionBase(i)));
        }
        if (hasValue(jsObject.allActions)) {
            let { buildDotNetActionBase } = await import('./actionBase');
            dotNetFeaturesViewModel.allActions = await Promise.all(jsObject.allActions.map(async i => await buildDotNetActionBase(i)));
        }
        if (hasValue(jsObject.defaultActions)) {
            let { buildDotNetActionBase } = await import('./actionBase');
            dotNetFeaturesViewModel.defaultActions = await Promise.all(jsObject.defaultActions.map(async i => await buildDotNetActionBase(i)));
        }
        ...
        if (hasValue(jsObject.goToOverride)) {
            let { buildDotNetGoToOverride } = await import('./goToOverride');
            dotNetFeaturesViewModel.goToOverride = await buildDotNetGoToOverride(jsObject.goToOverride);
        }
    ...
    if (hasValue(jsObject.featureViewModels)) {
        dotNetFeaturesViewModel.featureViewModels = jsObject.featureViewModels;
    }
