...
export async function buildDotNetListNode(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetListNodeGenerated } = await import('./listNode.gb');
    return await buildDotNetListNodeGenerated(jsObject, layerId, viewId);
}
