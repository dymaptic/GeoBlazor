...
export async function buildJsActiveLayerInfoGenerated(dotNetObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(dotNetObject.legendElements) && dotNetObject.legendElements.length > 0) {
        ...
        properties.legendElements = dotNetObject.legendElements.map(i => buildJsILegendElement(i, viewId)) as any;
    }
...
export async function buildDotNetActiveLayerInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.legendElements)) {
        ...
        dotNetActiveLayerInfo.legendElements = jsObject.legendElements.map(i => buildDotNetILegendElement(i, viewId));
    }
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetActiveLayerInfo.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetActiveLayerInfo.id)) {
        jsObjectRefs[dotNetActiveLayerInfo.id] ??= jsObject;
        arcGisObjectRefs[dotNetActiveLayerInfo.id] ??= jsObject;
    }
