...
export async function buildDotNetVirtualLightingGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetVirtualLighting.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetVirtualLighting.id)) {
        jsObjectRefs[dotNetVirtualLighting.id] ??= jsObject;
        arcGisObjectRefs[dotNetVirtualLighting.id] ??= jsObject;
    }
