...
export async function buildDotNetCompassViewModel(jsObject: any, viewId: string | null): Promise<any> {
    let {buildDotNetCompassViewModelGenerated} = await import('./compassViewModel.gb');
    return await buildDotNetCompassViewModelGenerated(jsObject, viewId);
}
