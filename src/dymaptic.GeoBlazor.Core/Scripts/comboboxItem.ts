...
export async function buildDotNetComboboxItem(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetComboboxItemGenerated } = await import('./comboboxItem.gb');
    return await buildDotNetComboboxItemGenerated(jsObject, layerId, viewId);
}
