...
export async function buildDotNetLegendWidget(jsObject: any, viewId: string | null): Promise<any> {
    let {buildDotNetLegendWidgetGenerated} = await import('./legendWidget.gb');
    return await buildDotNetLegendWidgetGenerated(jsObject, viewId);
}
