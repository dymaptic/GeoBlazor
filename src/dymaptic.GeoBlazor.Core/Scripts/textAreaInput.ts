...
export async function buildDotNetTextAreaInput(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetTextAreaInputGenerated } = await import('./textAreaInput.gb');
    return await buildDotNetTextAreaInputGenerated(jsObject, viewId);
}
