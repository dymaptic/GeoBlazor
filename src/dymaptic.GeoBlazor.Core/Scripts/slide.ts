...
export async function buildDotNetSlide(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetSlideGenerated } = await import('./slide.gb');
    return await buildDotNetSlideGenerated(jsObject, layerId, viewId);
}
