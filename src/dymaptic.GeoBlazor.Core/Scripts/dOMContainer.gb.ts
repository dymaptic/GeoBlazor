...
export async function buildJsDOMContainerGenerated(dotNetObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    let { buildDotNetDOMContainer } = await import('./dOMContainer');
    let dnInstantiatedObject = await buildDotNetDOMContainer(jsDOMContainer);
    ...
    try {
            ...
            jsObjectRef, JSON.stringify(dnInstantiatedObject, function (key, value) {
                ...
                if (typeof value === 'object' && value !== null) {
                    if (seenObjects.has(value)) {
                        console.warn(`Circular reference in serializing type DOMContainer detected at path: ${key}, value: ${value.__proto__?.declaredClass}`);
                        return undefined;
                    }
        ...
        console.error('Error invoking OnJsComponentCreated for DOMContainer', e);
    }
