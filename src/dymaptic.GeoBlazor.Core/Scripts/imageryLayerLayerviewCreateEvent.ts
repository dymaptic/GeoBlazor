...
export async function buildDotNetImageryLayerLayerviewCreateEvent(jsObject: any, viewId: string | null): Promise<any> {
    let {buildDotNetImageryLayerLayerviewCreateEventGenerated} = await import('./imageryLayerLayerviewCreateEvent.gb');
    return await buildDotNetImageryLayerLayerviewCreateEventGenerated(jsObject, viewId);
}
