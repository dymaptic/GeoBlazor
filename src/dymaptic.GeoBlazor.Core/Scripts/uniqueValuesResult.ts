...
export async function buildDotNetUniqueValuesResult(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetUniqueValuesResultGenerated } = await import('./uniqueValuesResult.gb');
    return await buildDotNetUniqueValuesResultGenerated(jsObject, layerId, viewId);
}
