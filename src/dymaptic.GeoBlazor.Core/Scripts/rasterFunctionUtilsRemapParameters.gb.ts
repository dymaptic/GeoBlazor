...
export async function buildDotNetRasterFunctionUtilsRemapParametersGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.rangeMaps)) {
        ...
        dotNetRasterFunctionUtilsRemapParameters.rangeMaps = await Promise.all(jsObject.rangeMaps.map(async i => await buildDotNetPixelValueRangeMap(i, layerId, viewId)));
    }
    ...
    if (hasValue(jsObject.outputPixelType)) {
        dotNetRasterFunctionUtilsRemapParameters.outputPixelType = removeCircularReferences(jsObject.outputPixelType);
    }
    
    if (hasValue(jsObject.raster)) {
        dotNetRasterFunctionUtilsRemapParameters.raster = removeCircularReferences(jsObject.raster);
    }
