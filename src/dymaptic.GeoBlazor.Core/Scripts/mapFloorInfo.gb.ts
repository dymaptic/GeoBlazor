...
export async function buildDotNetMapFloorInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.facilityLayer)) {
        ...
        dotNetMapFloorInfo.facilityLayer = await buildDotNetFacilityLayerInfo(jsObject.facilityLayer, viewId);
    }
    
    if (hasValue(jsObject.levelLayer)) {
        let { buildDotNetLevelLayerInfo } = await import('./levelLayerInfo');
        dotNetMapFloorInfo.levelLayer = await buildDotNetLevelLayerInfo(jsObject.levelLayer, viewId);
    }
    
    if (hasValue(jsObject.siteLayer)) {
        let { buildDotNetSiteLayerInfo } = await import('./siteLayerInfo');
        dotNetMapFloorInfo.siteLayer = await buildDotNetSiteLayerInfo(jsObject.siteLayer, viewId);
    }
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetMapFloorInfo.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetMapFloorInfo.id)) {
        jsObjectRefs[dotNetMapFloorInfo.id] ??= jsObject;
        arcGisObjectRefs[dotNetMapFloorInfo.id] ??= jsObject;
    }
