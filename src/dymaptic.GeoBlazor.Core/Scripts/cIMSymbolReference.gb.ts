...
import CIMSymbolReference = __esri.CIMSymbolReference;
...
export async function buildJsCIMSymbolReferenceGenerated(dotNetObject: any): Promise<any> {
    ...
    if (hasValue(dotNetObject.primitiveOverrides)) {
        const { id, dotNetComponentReference, layerId, viewId, ...sanitizedPrimitiveOverrides } = dotNetObject.primitiveOverrides;
        jsCIMSymbolReference.primitiveOverrides = sanitizedPrimitiveOverrides;
    }
    if (hasValue(dotNetObject.scaleDependentSizeVariation)) {
        const { id, dotNetComponentReference, layerId, viewId, ...sanitizedScaleDependentSizeVariation } = dotNetObject.scaleDependentSizeVariation;
