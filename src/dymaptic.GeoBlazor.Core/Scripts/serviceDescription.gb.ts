...
export async function buildDotNetServiceDescriptionGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.defaultTravelMode)) {
        ...
        dotNetServiceDescription.defaultTravelMode = await buildDotNetTravelMode(jsObject.defaultTravelMode);
    }
    if (hasValue(jsObject.supportedTravelModes)) {
        let { buildDotNetTravelMode } = await import('./travelMode');
        dotNetServiceDescription.supportedTravelModes = await Promise.all(jsObject.supportedTravelModes.map(async i => await buildDotNetTravelMode(i)));
    }
