...
export async function buildDotNetGeometryEngineAsync(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetGeometryEngineAsyncGenerated} = await import('./geometryEngineAsync.gb');
    return await buildDotNetGeometryEngineAsyncGenerated(jsObject, layerId, viewId);
}
