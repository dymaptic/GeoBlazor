...
export async function buildDotNetTessellatedMeshGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.vertices)) {
        ...
        dotNetTessellatedMesh.vertices = await Promise.all(jsObject.vertices.map(async i => await buildDotNetMeshVertex(i)));
    }
