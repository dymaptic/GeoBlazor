    ...
    switch (jsObject.type) {
        ...
        let dotNetFillSymbolProperties: any = {};
            if (hasValue(jsObject.color)) {
                dotNetFillSymbolProperties.color = removeCircularReferences(jsObject.color);
            }
    
            if (hasValue(jsObject.outline)) {
                dotNetFillSymbolProperties.outline = removeCircularReferences(jsObject.outline);
            }
    
            if (hasValue(jsObject.type)) {
                dotNetFillSymbolProperties.type = removeCircularReferences(jsObject.type);
            }
