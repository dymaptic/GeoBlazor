...
export async function buildDotNetSearch(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetSearchGenerated} = await import('./search.gb');
    return await buildDotNetSearchGenerated(jsObject, layerId, viewId);
}
