...
export async function buildDotNetIntegratedMesh3DTilesLayerElevationInfo(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetIntegratedMesh3DTilesLayerElevationInfoGenerated } = await import('./integratedMesh3DTilesLayerElevationInfo.gb');
    return await buildDotNetIntegratedMesh3DTilesLayerElevationInfoGenerated(jsObject, layerId, viewId);
}
