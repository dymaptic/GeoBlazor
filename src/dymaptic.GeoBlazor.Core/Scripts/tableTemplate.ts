...
export async function buildDotNetTableTemplate(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetTableTemplateGenerated } = await import('./tableTemplate.gb');
    return await buildDotNetTableTemplateGenerated(jsObject, layerId, viewId);
}
