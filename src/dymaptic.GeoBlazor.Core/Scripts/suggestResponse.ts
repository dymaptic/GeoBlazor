...
export async function buildDotNetSuggestResponse(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetSuggestResponseGenerated } = await import('./suggestResponse.gb');
    return await buildDotNetSuggestResponseGenerated(jsObject, viewId);
}
