...
export async function buildDotNetAreasAndLengthsParameters(jsObject: any): Promise<any> {
    let {buildDotNetAreasAndLengthsParametersGenerated} = await import('./areasAndLengthsParameters.gb');
    return await buildDotNetAreasAndLengthsParametersGenerated(jsObject);
}
