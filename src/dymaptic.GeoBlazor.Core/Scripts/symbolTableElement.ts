...
export async function buildDotNetSymbolTableElement(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetSymbolTableElementGenerated } = await import('./symbolTableElement.gb');
    return await buildDotNetSymbolTableElementGenerated(jsObject, layerId, viewId);
}
