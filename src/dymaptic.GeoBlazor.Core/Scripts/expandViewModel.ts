...
export async function buildDotNetExpandViewModel(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetExpandViewModelGenerated } = await import('./expandViewModel.gb');
    return await buildDotNetExpandViewModelGenerated(jsObject, viewId);
}
