...
import { buildDotNetWebsceneApplicationProperties } from './websceneApplicationProperties';
...
export async function buildDotNetWebsceneApplicationPropertiesGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        dotNetWebsceneApplicationProperties.id = geoBlazorId;
    } else if (hasValue(viewId)) {
        let dotNetRef = dotNetRefs[viewId!];
        dotNetWebsceneApplicationProperties.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetWebsceneApplicationProperties.id)) {
        jsObjectRefs[dotNetWebsceneApplicationProperties.id] ??= jsObject;
        arcGisObjectRefs[dotNetWebsceneApplicationProperties.id] ??= jsObject;
    }
