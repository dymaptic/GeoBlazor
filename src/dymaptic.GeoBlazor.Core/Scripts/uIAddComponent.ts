...
export async function buildDotNetUIAddComponent(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetUIAddComponentGenerated} = await import('./uIAddComponent.gb');
    return await buildDotNetUIAddComponentGenerated(jsObject, layerId, viewId);
}
