...
export async function buildDotNetImageHeightParameters(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetImageHeightParametersGenerated} = await import('./imageHeightParameters.gb');
    return await buildDotNetImageHeightParametersGenerated(jsObject, layerId, viewId);
}
