...
export async function buildDotNetImageIdentifyParameters(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetImageIdentifyParametersGenerated} = await import('./imageIdentifyParameters.gb');
    return await buildDotNetImageIdentifyParametersGenerated(jsObject, layerId, viewId);
}
