...
export async function buildJsPortalUserFetchItemsParamsGenerated(dotNetObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(dotNetObject.folder)) {
        ...
        jsPortalUserFetchItemsParams.folder = await buildJsPortalFolder(dotNetObject.folder) as any;
    }
    ...
    let { buildDotNetPortalUserFetchItemsParams } = await import('./portalUserFetchItemsParams');
            ...
            jsObjectRef, JSON.stringify(dnInstantiatedObject, function (key, value) {
                ...
                if (typeof value === 'object' && value !== null) {
                    if (seenObjects.has(value)) {
                        console.warn(`Circular reference in serializing type PortalUserFetchItemsParams detected at path: ${key}, value: ${value.__proto__?.declaredClass}`);
                        return undefined;
                    }
