...
export async function buildDotNetBookmarksViewModel(jsObject: any, viewId: string | null): Promise<any> {
    let {buildDotNetBookmarksViewModelGenerated} = await import('./bookmarksViewModel.gb');
    return await buildDotNetBookmarksViewModelGenerated(jsObject, viewId);
}
