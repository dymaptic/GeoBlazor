...
export async function buildDotNetCloudyWeather(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetCloudyWeatherGenerated } = await import('./cloudyWeather.gb');
    return await buildDotNetCloudyWeatherGenerated(jsObject, viewId);
}
