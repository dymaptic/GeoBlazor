...
export async function buildDotNetSliderViewModel(jsObject: any, viewId: string | null): Promise<any> {
    let {buildDotNetSliderViewModelGenerated} = await import('./sliderViewModel.gb');
    return await buildDotNetSliderViewModelGenerated(jsObject, viewId);
}
