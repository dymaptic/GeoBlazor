...
export async function buildDotNetSearchViewModelSearchResponseResults(jsObject: any): Promise<any> {
    let {buildDotNetSearchViewModelSearchResponseResultsGenerated} = await import('./searchViewModelSearchResponseResults.gb');
    return await buildDotNetSearchViewModelSearchResponseResultsGenerated(jsObject);
}
