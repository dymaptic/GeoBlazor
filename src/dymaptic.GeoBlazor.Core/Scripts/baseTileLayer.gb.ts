    ...
    switch (jsObject.type) {
        case 'base-tile': 
            ...
            if (hasValue(jsObject.effect)) {
                let { buildDotNetEffect } = await import('./effect');
                dotNetBaseTileLayer.effect = buildDotNetEffect(jsObject.effect, viewId);
            }
    
            if (hasValue(jsObject.fullExtent)) {
                let { buildDotNetExtent } = await import('./extent');
                dotNetBaseTileLayer.fullExtent = buildDotNetExtent(jsObject.fullExtent, viewId);
            }
    
            if (hasValue(jsObject.spatialReference)) {
                let { buildDotNetSpatialReference } = await import('./spatialReference');
                dotNetBaseTileLayer.spatialReference = buildDotNetSpatialReference(jsObject.spatialReference, viewId);
            }
            ...
            if (hasValue(jsObject.visibilityTimeExtent)) {
                let { buildDotNetTimeExtent } = await import('./timeExtent');
                dotNetBaseTileLayer.visibilityTimeExtent = buildDotNetTimeExtent(jsObject.visibilityTimeExtent, viewId);
            }
