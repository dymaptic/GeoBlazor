...
export async function buildJsSuggestResponseGenerated(dotNetObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(dotNetObject.errors) && dotNetObject.errors.length > 0) {
        let { buildJsSuggestResponseErrors } = await import('./suggestResponseErrors');
        jsSuggestResponse.errors = await Promise.all(dotNetObject.errors.map(async i => await buildJsSuggestResponseErrors(i, layerId, viewId))) as any;
    }
...
export async function buildDotNetSuggestResponseGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.errors)) {
        let { buildDotNetSuggestResponseErrors } = await import('./suggestResponseErrors');
        dotNetSuggestResponse.errors = await Promise.all(jsObject.errors.map(async i => await buildDotNetSuggestResponseErrors(i)));
    }
