...
export async function buildDotNetEdgeSourceJSONGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.assetGroups)) {
        ...
        dotNetEdgeSourceJSON.assetGroups = await Promise.all(jsObject.assetGroups.map(async i => await buildDotNetAssetGroupJSON(i)));
    }
