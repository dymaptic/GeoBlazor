...
export async function buildJsUniqueValueInfoGenerated(dotNetObject: any, viewId: string | null): Promise<any> {
...
export async function buildDotNetUniqueValueInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetUniqueValueInfo.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetUniqueValueInfo.id)) {
        jsObjectRefs[dotNetUniqueValueInfo.id] ??= jsObject;
        arcGisObjectRefs[dotNetUniqueValueInfo.id] ??= jsObject;
    }
