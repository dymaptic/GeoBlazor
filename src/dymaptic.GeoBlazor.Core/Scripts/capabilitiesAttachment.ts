...
export async function buildDotNetCapabilitiesAttachment(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetCapabilitiesAttachmentGenerated } = await import('./capabilitiesAttachment.gb');
    return await buildDotNetCapabilitiesAttachmentGenerated(jsObject, layerId, viewId);
}
