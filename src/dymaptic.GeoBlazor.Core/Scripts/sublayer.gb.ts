...
export default class SublayerGenerated implements IPropertyWrapper {
    ...
    async getFeatureType(feature: any): Promise<any> {
        return this.component.getFeatureType(feature);
    }
            ...
            jsObjectRef, JSON.stringify(dnInstantiatedObject, function (key, value) {
                if (typeof value === 'object' && value !== null) {
                    if (seenObjects.has(value)) {
                        console.warn(`Circular reference in serializing type Sublayer detected at path: ${key}, value: ${value}`);
                        return undefined;
                    }
...
export async function buildDotNetSublayerGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.popupTemplate)) {
        dotNetSublayer.popupTemplate = jsObject.popupTemplate;
    }
