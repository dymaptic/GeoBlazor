...
export async function buildDotNetRasterIdentifyOptions(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetRasterIdentifyOptionsGenerated} = await import('./rasterIdentifyOptions.gb');
    return await buildDotNetRasterIdentifyOptionsGenerated(jsObject, layerId, viewId);
}
