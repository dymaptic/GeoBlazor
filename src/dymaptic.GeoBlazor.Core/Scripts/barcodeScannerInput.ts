...
export async function buildDotNetBarcodeScannerInput(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetBarcodeScannerInputGenerated } = await import('./barcodeScannerInput.gb');
    return await buildDotNetBarcodeScannerInputGenerated(jsObject, viewId);
}
