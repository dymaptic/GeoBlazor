...
export async function buildDotNetPredominantCategoriesResult(jsObject: any): Promise<any> {
    let { buildDotNetPredominantCategoriesResultGenerated } = await import('./predominantCategoriesResult.gb');
    return await buildDotNetPredominantCategoriesResultGenerated(jsObject);
}
