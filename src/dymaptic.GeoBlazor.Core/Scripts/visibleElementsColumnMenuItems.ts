...
export async function buildDotNetVisibleElementsColumnMenuItems(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetVisibleElementsColumnMenuItemsGenerated } = await import('./visibleElementsColumnMenuItems.gb');
    return await buildDotNetVisibleElementsColumnMenuItemsGenerated(jsObject, viewId);
}
