...
export async function buildDotNetRasterIdentifyResultGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        let { buildJsRasterIdentifyResult } = await import('./rasterIdentifyResult');
        jsComponentRef = await buildJsRasterIdentifyResult(jsObject, layerId, viewId);
    }
    ...
    if (hasValue(jsObject.dataSeries)) {
        ...
        dotNetRasterIdentifyResult.dataSeries = await Promise.all(jsObject.dataSeries.map(async i => await buildDotNetRasterSliceValue(i, layerId, viewId)));
    }
