...
export default class SizeVariableGenerated implements IPropertyWrapper {
    ...
    async updateComponent(dotNetObject: any): Promise<void> {
        if (hasValue(dotNetObject.legendOptions)) {
            ...
            this.component.legendOptions = await buildJsVisualVariableLegendOptions(dotNetObject.legendOptions, this.viewId) as any;
        }
        if (hasValue(dotNetObject.stops) && dotNetObject.stops.length > 0) {
            let { buildJsSizeStop } = await import('./sizeStop');
            this.component.stops = await Promise.all(dotNetObject.stops.map(async i => await buildJsSizeStop(i, this.viewId))) as any;
        }
    ...
    async getLegendOptions(): Promise<any> {
        ...
        return await buildDotNetVisualVariableLegendOptions(this.component.legendOptions, this.viewId);
    }
    
    async setLegendOptions(value: any): Promise<void> {
        let { buildJsVisualVariableLegendOptions } = await import('./visualVariableLegendOptions');
        this.component.legendOptions = await  buildJsVisualVariableLegendOptions(value, this.viewId);
    }
    
    getNormalizationField(): any {
        if (!hasValue(this.component.normalizationField)) {
            return null;
        }
        
        return generateSerializableJson(this.component.normalizationField);
    }
    
    setNormalizationField(value: any): void {
        this.component.normalizationField = JSON.parse(value);
    }
    ...
    getTarget(): any {
        ...
        return generateSerializableJson(this.component.target);
    }
    
    setTarget(value: any): void {
        this.component.target = JSON.parse(value);
    }
...
export async function buildDotNetSizeVariableGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.axis)) {
        dotNetSizeVariable.axis = removeCircularReferences(jsObject.axis);
    }
    ...
    if (hasValue(jsObject.maxSize)) {
        dotNetSizeVariable.maxSize = removeCircularReferences(jsObject.maxSize);
    }
    ...
    if (hasValue(jsObject.minSize)) {
        dotNetSizeVariable.minSize = removeCircularReferences(jsObject.minSize);
    }
    ...
    if (hasValue(jsObject.type)) {
        dotNetSizeVariable.type = removeCircularReferences(jsObject.type);
    }
    ...
    if (hasValue(jsObject.valueRepresentation)) {
        dotNetSizeVariable.valueRepresentation = removeCircularReferences(jsObject.valueRepresentation);
    }
