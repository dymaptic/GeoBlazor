...
export async function buildDotNetTrackInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.fields)) {
        ...
        dotNetTrackInfo.fields = await Promise.all(jsObject.fields.map(async i => await buildDotNetAggregateField(i, viewId)));
    }
    ...
    if (hasValue(jsObject.maxDisplayDuration)) {
        ...
        dotNetTrackInfo.maxDisplayDuration = await buildDotNetTimeInterval(jsObject.maxDisplayDuration, viewId);
    }
    
    if (hasValue(jsObject.popupTemplate)) {
        let { buildDotNetPopupTemplate } = await import('./popupTemplate');
        dotNetTrackInfo.popupTemplate = await buildDotNetPopupTemplate(jsObject.popupTemplate, viewId);
    }
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetTrackInfo.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetTrackInfo.id)) {
        jsObjectRefs[dotNetTrackInfo.id] ??= jsObject;
        arcGisObjectRefs[dotNetTrackInfo.id] ??= jsObject;
    }
