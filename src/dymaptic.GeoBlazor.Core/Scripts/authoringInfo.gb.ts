...
export async function buildJsAuthoringInfoGenerated(dotNetObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(dotNetObject.colorRamp)) {
        ...
        properties.colorRamp = buildJsColorRamp(dotNetObject.colorRamp, viewId) as any;
    }
    if (hasValue(dotNetObject.field1)) {
        let { buildJsAuthoringInfoField } = await import('./authoringInfoField');
        properties.field1 = await buildJsAuthoringInfoField(dotNetObject.field1, viewId) as any;
    }
    if (hasValue(dotNetObject.field2)) {
        let { buildJsAuthoringInfoField } = await import('./authoringInfoField');
        properties.field2 = await buildJsAuthoringInfoField(dotNetObject.field2, viewId) as any;
    }
    if (hasValue(dotNetObject.statistics)) {
        let { buildJsAuthoringInfoStatistics } = await import('./authoringInfoStatistics');
        properties.statistics = await buildJsAuthoringInfoStatistics(dotNetObject.statistics, viewId) as any;
    }
    if (hasValue(dotNetObject.visualVariables) && dotNetObject.visualVariables.length > 0) {
        ...
        properties.visualVariables = await Promise.all(dotNetObject.visualVariables.map(async i => await buildJsAuthoringInfoVisualVariable(i, viewId))) as any;
    }
...
export async function buildDotNetAuthoringInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.colorRamp)) {
        ...
        dotNetAuthoringInfo.colorRamp = buildDotNetColorRamp(jsObject.colorRamp, viewId);
    }
    
    if (hasValue(jsObject.field1)) {
        let { buildDotNetAuthoringInfoField } = await import('./authoringInfoField');
        dotNetAuthoringInfo.field1 = await buildDotNetAuthoringInfoField(jsObject.field1, viewId);
    }
    
    if (hasValue(jsObject.field2)) {
        let { buildDotNetAuthoringInfoField } = await import('./authoringInfoField');
        dotNetAuthoringInfo.field2 = await buildDotNetAuthoringInfoField(jsObject.field2, viewId);
    }
    
    if (hasValue(jsObject.statistics)) {
        let { buildDotNetAuthoringInfoStatistics } = await import('./authoringInfoStatistics');
        dotNetAuthoringInfo.statistics = await buildDotNetAuthoringInfoStatistics(jsObject.statistics, viewId);
    }
    
    if (hasValue(jsObject.visualVariables)) {
        let { buildDotNetAuthoringInfoVisualVariable } = await import('./authoringInfoVisualVariable');
        dotNetAuthoringInfo.visualVariables = await Promise.all(jsObject.visualVariables.map(async i => await buildDotNetAuthoringInfoVisualVariable(i, viewId)));
    }
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetAuthoringInfo.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetAuthoringInfo.id)) {
        jsObjectRefs[dotNetAuthoringInfo.id] ??= jsObject;
        arcGisObjectRefs[dotNetAuthoringInfo.id] ??= jsObject;
    }
