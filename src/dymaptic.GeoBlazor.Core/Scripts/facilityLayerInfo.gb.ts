...
export async function buildDotNetFacilityLayerInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetFacilityLayerInfo.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetFacilityLayerInfo.id)) {
        jsObjectRefs[dotNetFacilityLayerInfo.id] ??= jsObject;
        arcGisObjectRefs[dotNetFacilityLayerInfo.id] ??= jsObject;
    }
