...
export async function buildDotNetCoverageDescriptionV201EoMetadata(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetCoverageDescriptionV201EoMetadataGenerated } = await import('./coverageDescriptionV201EoMetadata.gb');
    return await buildDotNetCoverageDescriptionV201EoMetadataGenerated(jsObject, viewId);
}
