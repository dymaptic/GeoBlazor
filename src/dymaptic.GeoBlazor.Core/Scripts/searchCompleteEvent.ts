...
export async function buildDotNetSearchCompleteEvent(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetSearchCompleteEventGenerated } = await import('./searchCompleteEvent.gb');
    return await buildDotNetSearchCompleteEventGenerated(jsObject, viewId);
}
