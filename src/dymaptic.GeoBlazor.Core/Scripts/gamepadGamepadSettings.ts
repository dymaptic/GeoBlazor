...
export async function buildDotNetGamepadGamepadSettings(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetGamepadGamepadSettingsGenerated } = await import('./gamepadGamepadSettings.gb');
    return await buildDotNetGamepadGamepadSettingsGenerated(jsObject, layerId, viewId);
}
