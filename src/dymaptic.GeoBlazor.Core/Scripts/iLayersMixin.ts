...
export async function buildDotNetILayersMixin(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetILayersMixinGenerated } = await import('./iLayersMixin.gb');
    return await buildDotNetILayersMixinGenerated(jsObject, layerId, viewId);
}
