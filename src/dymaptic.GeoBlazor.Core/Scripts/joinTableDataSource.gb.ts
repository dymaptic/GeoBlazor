...
export async function buildJsJoinTableDataSourceGenerated(dotNetObject: any): Promise<any> {
    ...
    if (hasValue(dotNetObject.leftTableSource)) {
        ...
        jsJoinTableDataSource.leftTableSource = await buildJsDynamicLayer(dotNetObject.leftTableSource) as any;
    }
    if (hasValue(dotNetObject.rightTableSource)) {
        let { buildJsDynamicLayer } = await import('./dynamicLayer');
        jsJoinTableDataSource.rightTableSource = await buildJsDynamicLayer(dotNetObject.rightTableSource) as any;
    }
    ...
    if (hasValue(geoBlazorId)) {
        ...
        if (hasValue(dotNetRef)) {
            try {
                ...
                console.error('Error invoking GetId for JoinTableDataSource', e);
            }
        }
    }
