...
export async function buildDotNetStylePattern3D(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetStylePattern3DGenerated } = await import('./stylePattern3D.gb');
    return await buildDotNetStylePattern3DGenerated(jsObject, layerId, viewId);
}
