...
export async function buildDotNetWeatherVisibleElements(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetWeatherVisibleElementsGenerated } = await import('./weatherVisibleElements.gb');
    return await buildDotNetWeatherVisibleElementsGenerated(jsObject, layerId, viewId);
}
