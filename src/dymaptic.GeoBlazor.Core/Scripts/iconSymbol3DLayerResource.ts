...
export async function buildDotNetIconSymbol3DLayerResource(jsObject: any): Promise<any> {
    let { buildDotNetIconSymbol3DLayerResourceGenerated } = await import('./iconSymbol3DLayerResource.gb');
    return await buildDotNetIconSymbol3DLayerResourceGenerated(jsObject);
}
