...
export async function buildDotNetDynamicDataLayerFields(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetDynamicDataLayerFieldsGenerated } = await import('./dynamicDataLayerFields.gb');
    return await buildDotNetDynamicDataLayerFieldsGenerated(jsObject, viewId);
}
