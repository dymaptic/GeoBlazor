            ...
            jsObjectRef, JSON.stringify(dnInstantiatedObject, function (key, value) {
                if (typeof value === 'object' && value !== null) {
                    if (seenObjects.has(value)) {
                        console.warn(`Circular reference in serializing type SearchViewModelSearchResponseResults detected at path: ${key}, value: ${value}`);
                        return undefined;
                    }
...
export async function buildDotNetSearchViewModelSearchResponseResultsGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.results)) {
        ...
        dotNetSearchViewModelSearchResponseResults.results = await Promise.all(jsObject.results.map(async i => await buildDotNetSearchResult(i)));
    }
