...
export async function buildDotNetSearchCompleteEventResults(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetSearchCompleteEventResultsGenerated } = await import('./searchCompleteEventResults.gb');
    return await buildDotNetSearchCompleteEventResultsGenerated(jsObject, layerId, viewId);
}
