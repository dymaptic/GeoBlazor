...
export async function buildDotNetValuePickerCollection(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetValuePickerCollectionGenerated } = await import('./valuePickerCollection.gb');
    return await buildDotNetValuePickerCollectionGenerated(jsObject, layerId, viewId);
}
