...
export async function buildDotNetMagnifierGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        let { buildJsMagnifier } = await import('./magnifier');
        jsComponentRef = await buildJsMagnifier(jsObject, layerId, viewId);
    }
    ...
    if (hasValue(jsObject.offset)) {
        ...
        dotNetMagnifier.offset = await buildDotNetMagnifierScreenPoint(jsObject.offset, layerId, viewId);
    }
    if (hasValue(jsObject.position)) {
        let { buildDotNetMagnifierScreenPoint } = await import('./magnifierScreenPoint');
        dotNetMagnifier.position = await buildDotNetMagnifierScreenPoint(jsObject.position, layerId, viewId);
    }
