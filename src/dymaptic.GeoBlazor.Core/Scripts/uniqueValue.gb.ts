...
export async function buildJsUniqueValueGenerated(dotNetObject: any, viewId: string | null): Promise<any> {
...
export async function buildDotNetUniqueValueGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetUniqueValue.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetUniqueValue.id)) {
        jsObjectRefs[dotNetUniqueValue.id] ??= jsObject;
        arcGisObjectRefs[dotNetUniqueValue.id] ??= jsObject;
    }
