...
export async function buildDotNetSwitchInput(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let { buildDotNetSwitchInputGenerated } = await import('./switchInput.gb');
    return await buildDotNetSwitchInputGenerated(jsObject, layerId, viewId);
}
