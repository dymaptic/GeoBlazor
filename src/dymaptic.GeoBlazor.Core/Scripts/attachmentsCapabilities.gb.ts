            ...
            jsObjectRef, JSON.stringify(dnInstantiatedObject, function (key, value) {
                ...
                if (typeof value === 'object' && value !== null
                    ...
                    if (seenObjects.has(value)) {
                        console.warn(`Circular reference in serializing type AttachmentsCapabilities detected at path: ${key}, value: ${value.declaredClass}`);
                        return undefined;
                    }
...
export async function buildDotNetAttachmentsCapabilitiesGenerated(jsObject: any): Promise<any> {
    ...
    if (hasValue(jsObject.operations)) {
        ...
        dotNetAttachmentsCapabilities.operations = await Promise.all(jsObject.operations.map(async i => await buildDotNetAttachmentsCapabilitiesOperations(i)));
    }
