...
export async function buildDotNetEditedFeatureResult(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetEditedFeatureResultGenerated} = await import('./editedFeatureResult.gb');
    return await buildDotNetEditedFeatureResultGenerated(jsObject, layerId, viewId);
}
