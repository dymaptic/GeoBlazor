...
export async function buildDotNetDisplayFilterInfoGenerated(jsObject: any, viewId: string | null): Promise<any> {
    ...
    if (hasValue(geoBlazorId)) {
        ...
        dotNetDisplayFilterInfo.id = await dotNetRef.invokeMethodAsync('GetId');
    }
    if (hasValue(dotNetDisplayFilterInfo.id)) {
        jsObjectRefs[dotNetDisplayFilterInfo.id] ??= jsObject;
        arcGisObjectRefs[dotNetDisplayFilterInfo.id] ??= jsObject;
    }
