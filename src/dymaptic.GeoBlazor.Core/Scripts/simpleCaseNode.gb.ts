...
export async function buildDotNetSimpleCaseNodeGenerated(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    ...
    if (hasValue(jsObject.clauses)) {
        ...
        dotNetSimpleCaseNode.clauses = await Promise.all(jsObject.clauses.map(async i => await buildDotNetWhenNode(i, layerId, viewId)));
    }
    
    if (hasValue(jsObject.else)) {
        dotNetSimpleCaseNode.else = removeCircularReferences(jsObject.else);
    }
    ...
    if (hasValue(jsObject.operand)) {
        dotNetSimpleCaseNode.operand = removeCircularReferences(jsObject.operand);
    }
