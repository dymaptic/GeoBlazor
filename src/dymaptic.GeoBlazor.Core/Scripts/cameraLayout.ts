...
export async function buildDotNetCameraLayout(jsObject: any, viewId: string | null): Promise<any> {
    let { buildDotNetCameraLayoutGenerated } = await import('./cameraLayout.gb');
    return await buildDotNetCameraLayoutGenerated(jsObject, viewId);
}
