...
export async function buildDotNetBookmarksWidget(jsObject: any, layerId: string | null, viewId: string | null): Promise<any> {
    let {buildDotNetBookmarksWidgetGenerated} = await import('./bookmarksWidget.gb');
    return await buildDotNetBookmarksWidgetGenerated(jsObject, layerId, viewId);
}
