...
export async function buildDotNetObjectSymbol3DLayerResource(jsObject: any): Promise<any> {
    let { buildDotNetObjectSymbol3DLayerResourceGenerated } = await import('./objectSymbol3DLayerResource.gb');
    return await buildDotNetObjectSymbol3DLayerResourceGenerated(jsObject);
}
