...
#pragma warning restore BL0005
    ...
    public async Task<ExtentQueryResult?> QueryExtent(Query query, CancellationToken cancellationToken = default)
    {
        ...
        ExtentQueryResult? result = await JsComponentReference.InvokeJsMethod<ExtentQueryResult?>(IsServer,
            nameof(QueryExtent), nameof(FeatureLayerView), View?.QueryResultsMaxSizeLimit, 
            cancellationToken, query, abortSignal);

        if (result?.Extent is not null)
        {
            result.Extent.UpdateGeoBlazorReferences(CoreJsModule!, ProJsModule, View, null, Layer);
        }
    ...
    public async Task<int?> QueryFeatureCount(Query? query = null, CancellationToken cancellationToken = default)
    {
        ...
        int? result = await JsComponentReference.InvokeJsMethod<int?>(IsServer, nameof(QueryFeatureCount),
            nameof(FeatureLayerView), View?.QueryResultsMaxSizeLimit, cancellationToken, query, abortSignal);
        await AbortManager.DisposeAbortController(cancellationToken);
    ...
    public async Task<FeatureSet?> QueryFeatures(Query? query = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            graphic.UpdateGeoBlazorReferences(CoreJsModule!, ProJsModule, View, Layer, Layer);
        }
    ...
    public async Task<ObjectId[]?> QueryObjectIds(Query query, CancellationToken cancellationToken = default)
    {
        ...
        ObjectId[]? queryResult = await JsComponentReference.InvokeJsMethod<ObjectId[]?>(IsServer,
            nameof(QueryObjectIds), nameof(FeatureLayerView), View?.QueryResultsMaxSizeLimit, 
            cancellationToken, query, abortSignal);
