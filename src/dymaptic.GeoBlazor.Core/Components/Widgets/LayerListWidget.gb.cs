...
#region Property Getters
    ...
    public async Task<ICatalogLayerListWidget?> GetCatalogLayerList()
    {
        ...
        ICatalogLayerListWidget? result = await JsComponentReference.InvokeAsync<ICatalogLayerListWidget?>(
            "getCatalogLayerList", CancellationTokenSource.Token);
        
        if (result is not null)
        {
            if (CatalogLayerList is not null)
            {
                result.Id = CatalogLayerList.Id;
            }
...
#pragma warning restore BL0005
    ...
    public async Task<ITableListWidget?> GetTableList()
    {
        ...
        ITableListWidget? result = await JsComponentReference.InvokeAsync<ITableListWidget?>(
            "getTableList", CancellationTokenSource.Token);
        
        if (result is not null)
        {
            if (TableList is not null)
            {
                result.Id = TableList.Id;
            }
...
#region Event Handlers
    ...
    protected override async ValueTask<bool> RegisterGeneratedChildComponent(MapComponent child)
    {
        switch (child)
        {
            case ICatalogLayerListWidget catalogLayerList:
                if (catalogLayerList != CatalogLayerList)
                {
                ...
                return true;
            case ITableListWidget tableList:
    ...
    protected override async ValueTask<bool> UnregisterGeneratedChildComponent(MapComponent child)
    {
        switch (child)
        {
            case ICatalogLayerListWidget _:
                CatalogLayerList = null;
                WidgetChanged = true;
                ModifiedParameters[nameof(CatalogLayerList)] = CatalogLayerList;
                return true;
            case ITableListWidget _:
                TableList = null;
                WidgetChanged = true;
                ModifiedParameters[nameof(TableList)] = TableList;
                return true;
    ...
    public override void ValidateRequiredGeneratedChildren()
    {
        ...
        CatalogLayerList?.ValidateRequiredGeneratedChildren();
        TableList?.ValidateRequiredGeneratedChildren();
