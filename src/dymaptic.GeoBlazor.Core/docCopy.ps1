...
Get-ChildItem -Path $SourcePath -Filter "*.md" |
        ForEach {
            ...
            "$( $FrontMatter )$( $_.BaseName -eq 'index' ? $Index : $Title )$( $Parent )Classes$( $EndMatter )$( Get-Content $_.FullName -raw )" -replace '.md', '.html' -replace '>[\s]*ArcGIS[\s]*JS[\s]*API[\s]*</a>', '>ArcGIS JS API</a>' |
