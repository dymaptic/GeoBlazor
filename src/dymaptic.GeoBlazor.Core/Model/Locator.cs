...
public class Locator : LogicComponent
{
    ...
    {
        return await AddressesToLocations(ESRIGeoLocationUrl, addresses, countryCode, categories, locationType, outSpatialReference);
    }
    ...
    /// <param name="outSpatialReference">The spatial reference of the output geometries. If not specified, the output geometries are in the spatial reference of the input geometries when performing a reverse geocode and in the default spatial reference returned by the service if finding locations by address.</param>
    /// <returns></returns>
    public async Task<List<AddressCandidate>> AddressesToLocations(string url, List<Address> addresses, string? countryCode = null, List<string>? categories = null, string? locationType = null, SpatialReference? outSpatialReference = null)
    {
        return await InvokeAsync<List<AddressCandidate>>("addressesToLocations", url, addresses, countryCode, categories, locationType, outSpatialReference);
    }
    ...
    {
        return await AddressToLocations(ESRIGeoLocationUrl, address, categories, countryCode, forStorage, location, locationType, magicKey, maxLocations, outFields, outSpatialReference, searchExtent);
    }
    ...
    {
        return await InvokeAsync<List<AddressCandidate>>("addressToLocations", url, address, categories, countryCode, forStorage, location, locationType, magicKey, maxLocations, outFields, outSpatialReference, searchExtent);
    }
    ...
    /// <param name="outSpatialReference">The spatial reference of the output geometries. If not specified, the output geometries are in the spatial reference of the input geometries when performing a reverse geocode and in the default spatial reference returned by the service if finding locations by address.</param>
    /// <returns></returns>
    public async Task<AddressCandidate> LocationToAddress(Point location, string? locationType = null,
    ...
    /// <param name="location">The point at which to search for the closest address. The location should be in the same spatial reference as that of the geocode service.</param>
    /// <param name="locationType">Define the type of location, either "street" or "rooftop", of the point returned from the World Geocoding Service.</param>
    /// <param name="outSpatialReference">The spatial reference of the output geometries. If not specified, the output geometries are in the spatial reference of the input geometries when performing a
    /// reverse geocode and in the default spatial reference returned by the service if finding locations by address.</param>
    public async Task<AddressCandidate> LocationToAddress(string url, Point location, string? locationType = null, SpatialReference? outSpatialReference = null)
    {
        return await InvokeAsync<AddressCandidate>("locationToAddress", url, location, locationType, outSpatialReference);
    }
    ...
    /// <param name="text">The input text entered by a user which is used by the suggest operation to generate a list of possible matches.</param>
    /// <param name="categories">A place or address type which can be used to filter suggest results. The parameter supports input of single category values or multiple comma-separated values.</param>
    public async Task<List<SuggestionResult>> SuggestLocations(string url, Point location, string text, List<string>? categories = null)
    {
        return await InvokeAsync<List<SuggestionResult>>("suggestLocations", url, location, text, categories);
    }

    private static string ESRIGeoLocationUrl = "https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer";
}
