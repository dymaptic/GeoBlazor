namespace dymaptic.GeoBlazor.Core.Functions;

/// <summary>
///     <a target="_blank" href="https://docs.geoblazor.com/pages/classes/dymaptic.GeoBlazor.Core.Functions.GetAllHeaders.html">GeoBlazor Docs</a>
///     A function for retrieving all headers sent from the server.
///     <a target="_blank" href="https://developers.arcgis.com/javascript/latest/api-reference/esri-request.html#EsriErrorDetails">ArcGIS Maps SDK for JavaScript</a>
/// </summary>
/// <param name="JavaScriptFunction">
///     The JavaScript function to call, passed as a string.
/// </param>
public record GetAllHeaders(string JavaScriptFunction);

