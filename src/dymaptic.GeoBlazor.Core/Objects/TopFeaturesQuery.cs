...
{
    /// <summary>
    ///     Indicates if the service should cache the query results. It only applies if the layer's capabilities.queryTopFeatures.supportsCacheHint is set to true. Use only for queries that have the same parameters every time the app is used.
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    ///     Known Limitations: If specifying outFields as expressions on a feature service-based FeatureLayer, the service capabilities advancedQueryCapabilities.supportsOutFieldSQLExpression and useStandardizedQueries must both be true.
    /// </remarks>
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
